class Interval:
    ...
    def __sub__(self, other_interval):
        ...
        left_end = min(self.end, other_interval.start)

        right_start = max(self.start, other_interval.end)
        right_end = self.end

        result = []

        if left_start < left_end:
            result.append(Interval(left_start, left_end))
        
        if right_start < right_end:
            result.append(Interval(right_start, right_end))
...
difference_result_3 = interval1 - interval3

print("Difference Result:", [str(interval) for interval in difference_result_1])  # [1, 3]
print("Difference Result:", [str(interval) for interval in difference_result_2])  # [5, 8]
print("Difference Result:", difference_result_3)  # None